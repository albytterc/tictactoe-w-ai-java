...
public class Board implements Cloneable {
    ...
    public Coords findBestMove(Board b, Player player) {
        ...
        for (Coords coords : freeMoves) {
            ...
            int score = minimax(b, getOpponent(player), coords, false, player, 1);
            if (score > bestScore) {
                bestScore = score;
                move = coords;
            }
...
//        }
    ...
    public int minimax(Board b, Player player, Coords move, boolean isMaximizing, Player startPlayer, int depth) {
        if (b.gameOver(player, move)) {
            if (b.playerX.isWinner()) {
                return startPlayer.getTypeChar() == 'X' ? 10 - depth : depth - 10;
            } else if (b.playerO.isWinner()) {
                return startPlayer.getTypeChar() == 'O' ? 10 - depth : depth - 10;
            } else {
                return 0;
            }
...
//            }
        ...
        for (Coords coords : emptySpots) {
            b.board[coords.getDown()][coords.getAcross()] = player.getTypeChar();
            int score = minimax(b, getOpponent(player), coords, !isMaximizing, startPlayer, depth + 1);
